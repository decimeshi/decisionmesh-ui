...
import {
  ...
  Clock, Layers, Loader2, ClipboardCheck,
} from 'lucide-react';
      ...
      <div className="flex gap-3">
        <div className="flex flex-col items-center shrink-0">
          <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2"
            style={{ background: decisionBg, borderColor: decisionBorder, color: decisionColor }}>
        ...
        <div className="flex-1 pb-4">
          <div className="flex items-start justify-between gap-2">
            <div className="flex-1">
              ...
              {entry.reason && (
                <p className="mt-1.5 text-[13px] text-slate-600 leading-relaxed">{entry.reason}</p>
              )}
...
function ReplaySummary({ data }) {
  ...
  return (
    <div className="flex items-center gap-3 flex-wrap p-3 rounded-lg border mb-4"
      style={{ background: chainValid ? '#f0fdf4' : '#fef2f2', borderColor: chainValid ? '#bbf7d0' : '#fecaca' }}>
      {chainValid
      ...
      <span className="text-[10px] font-mono px-2 py-1 rounded"
        style={{ background: chainValid ? '#dcfce7' : '#fee2e2', color: chainValid ? '#15803d' : '#b91c1c' }}>
...
export default function ReplayPanel({ intentId, keycloak, satisfactionState }) {
  const [state, setState] = useState('idle'); // idle | loading | done | error | pending_review
  const [data, setData]   = useState(null);
  const [error, setError] = useState(null);

  async function runReplay() {
    // Short-circuit — no API call needed for pending review intents
    if (satisfactionState === 'UNKNOWN') {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          >
            ...
            {state === 'pending_review' && <><RotateCcw size={12} /> Refresh</>}
          </Button>
        </div>
      </CardHeader>
      ...
      <CardContent>
        ...
        {state === 'pending_review' && (
          <div className="py-6 text-center">
            ...
            <p className="text-[13px] font-semibold text-slate-700">Awaiting Human Review</p>
            <p className="text-[12px] text-slate-400 mt-1 max-w-xs mx-auto leading-relaxed">
              This intent is pending a decision in the Review Queue.
              Replay will be available after the intent is approved or rejected.
            </p>
            <a href="/review-queue"
              className="inline-flex items-center gap-1.5 mt-3 text-[12px] text-blue-600 hover:text-blue-700 font-medium">
            ...
            {data.entries && data.entries.length > 0 ? (
              <div className="space-y-0">
                {data.entries.map((entry, i) => (
                  <ReplayEntry key={entry.aggregateVersion ?? i} entry={entry} index={i} isFirst={i === 0} />
                ))}
              </div>
            ) : (
              <div className="py-6 text-center">
                <p className="text-[13px] text-slate-500">No governance decisions recorded for this intent.</p>
